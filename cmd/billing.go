...
func (c *config) excel() error {
	...
	genericcli.Must(f.SetCellValue(postgresSheetName, "D1", "Postgres ID"))
	genericcli.Must(f.SetColWidth(postgresSheetName, "D", "D", 0))
	genericcli.Must(f.SetCellValue(postgresSheetName, "E1", "Description"))
	genericcli.Must(f.SetCellValue(postgresSheetName, "F1", "Start"))
	genericcli.Must(f.SetColWidth(postgresSheetName, "F", "F", 15))
	genericcli.Must(f.SetCellValue(postgresSheetName, "G1", "CPU[CPU*s]"))
	genericcli.Must(f.SetCellValue(postgresSheetName, "H1", "Memory[GiB*h]"))
	genericcli.Must(f.SetCellValue(postgresSheetName, "I1", "Storage[GiB*h]"))
	genericcli.Must(f.SetCellValue(postgresSheetName, "J1", "Lifetime[h]"))
	for i, v := range responsePostgres.Payload.Usage {
		...
		genericcli.Must(f.SetCellValue(postgresSheetName, "D"+fmt.Sprint(i+2), *v.Postgresid))
		genericcli.Must(f.SetCellValue(postgresSheetName, "E"+fmt.Sprint(i+2), *v.Postgresdescription))
		start := time.Time(*v.Postgresstart)
		genericcli.Must(f.SetCellValue(postgresSheetName, "F"+fmt.Sprint(i+2), start))
		cpuseconds, _ := strconv.Atoi(*v.Cpuseconds)
		genericcli.Must(f.SetCellValue(postgresSheetName, "G"+fmt.Sprint(i+2), cpuseconds))
		memoryhours := byteSecondsToGiBHours(*v.Memoryseconds)
		genericcli.Must(f.SetCellValue(postgresSheetName, "H"+fmt.Sprint(i+2), memoryhours))
		storagehours := byteSecondsToGiBHours(*v.Storageseconds)
		genericcli.Must(f.SetCellValue(postgresSheetName, "I"+fmt.Sprint(i+2), storagehours))
		genericcli.Must(f.SetCellValue(postgresSheetName, "J"+fmt.Sprint(i+2), *v.Lifetime/1000000000/3600))
	}
