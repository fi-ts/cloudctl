...
func (c *config) excel() error {
	...
	f.SetColWidth(clusterSheetName, "F", "F", 15)
	...
	for i, v := range responseCluster.Payload.Usage {
		...
		f.SetCellFormula(clusterSheetName, "I"+fmt.Sprint(i+2), "=MAX(H"+fmt.Sprint(i+2)+"-Parameter!B7*G"+fmt.Sprint(i+2)+"/3600,0)")
		...
		f.SetCellFormula(clusterSheetName, "L"+fmt.Sprint(i+2), "=MAX(K"+fmt.Sprint(i+2)+"-Parameter!B8*G"+fmt.Sprint(i+2)+"/3600,0)")
		...
		f.SetCellFormula(clusterSheetName, "O"+fmt.Sprint(i+2), "=MAX(N"+fmt.Sprint(i+2)+"-Parameter!B9*G"+fmt.Sprint(i+2)+"/3600,0)")
	...
	f.SetColWidth(containerSheetName, "K", "K", 15)
	...
	f.SetColWidth(volumeSheetName, "J", "J", 15)
	...
	for i, v := range responseVolume.Payload.Usage {
		...
		f.SetCellFormula(volumeSheetName, "M"+fmt.Sprint(i+2), "=IF(LEFT(H"+fmt.Sprint(i+2)+",7)=\"csi-lvm\",L"+fmt.Sprint(i+2)+",0)")
		f.SetCellFormula(volumeSheetName, "N"+fmt.Sprint(i+2), "=IF(LEFT(H"+fmt.Sprint(i+2)+",10)=\"partition-\",L"+fmt.Sprint(i+2)+",0)")
	}
	...
	f.SetColWidth(postgresSheetName, "F", "F", 15)
