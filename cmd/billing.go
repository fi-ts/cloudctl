...
func (c *config) excel() error {
	...
	genericcli.Must(f.SetCellValue(volumeSheetName, "I1", "Annotations"))
	genericcli.Must(f.SetColWidth(volumeSheetName, "I", "I", 0))
	genericcli.Must(f.SetCellValue(volumeSheetName, "J1", "Start"))
	genericcli.Must(f.SetColWidth(volumeSheetName, "J", "J", 0))
	genericcli.Must(f.SetCellValue(volumeSheetName, "K1", "Lifetime[h]"))
	genericcli.Must(f.SetCellValue(volumeSheetName, "L1", "Capacity[GiB*h]"))
	genericcli.Must(f.SetCellValue(volumeSheetName, "M1", "Local[GiB*h]"))
	genericcli.Must(f.SetCellValue(volumeSheetName, "N1", "Block[GiB*h]"))
	for i, v := range responseVolume.Payload.Usage {
		...
		genericcli.Must(f.SetCellValue(volumeSheetName, "I"+fmt.Sprint(i+2), strings.Join(v.Annotations, "; ")))
		start := time.Time(*v.Start)
		genericcli.Must(f.SetCellValue(volumeSheetName, "J"+fmt.Sprint(i+2), start))
		genericcli.Must(f.SetCellValue(volumeSheetName, "K"+fmt.Sprint(i+2), *v.Lifetime/1000000000/3600))
		capacityhours := byteSecondsToGiBHours(*v.Capacityseconds)
		genericcli.Must(f.SetCellValue(volumeSheetName, "L"+fmt.Sprint(i+2), capacityhours))
		genericcli.Must(f.SetCellFormula(volumeSheetName, "M"+fmt.Sprint(i+2), "=IF(ISNUMBER(SEARCH(\"replica\",I"+fmt.Sprint(i+2)+")),0,L"+fmt.Sprint(i+2)+")"))
		genericcli.Must(f.SetCellFormula(volumeSheetName, "N"+fmt.Sprint(i+2), "=IF(ISNUMBER(SEARCH(\"replica\",I"+fmt.Sprint(i+2)+")),L"+fmt.Sprint(i+2)+",0)"))
	}
	...
	genericcli.Must(f.SetCellValue(postgresSheetName, "H1", "CPU[CPU*h]"))
	...
	for i, v := range responsePostgres.Payload.Usage {
		...
		cpuseconds, _ := strconv.Atoi(*v.Cpuseconds)
		genericcli.Must(f.SetCellValue(postgresSheetName, "H"+fmt.Sprint(i+2), cpuseconds/3600))
		memoryhours := byteSecondsToGiBHours(*v.Memoryseconds)
