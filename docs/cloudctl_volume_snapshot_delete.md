## cloudctl volume snapshot delete

deletes the snapshot

```
cloudctl volume snapshot delete <id> [flags]
```
...
```
  ...
  -f, --file string      filename of the create or update request in yaml format, or - for stdin.
                         ...
                         Example:
                         $ cloudctl snapshot describe snapshot-1 -o yaml > snapshot.yaml
                         $ vi snapshot.yaml
                         $ # either via stdin
                         $ cat snapshot.yaml | cloudctl snapshot delete -f -
                         $ # or via file
                         $ cloudctl snapshot delete -f snapshot.yaml
