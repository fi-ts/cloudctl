## cloudctl volume snapshot list

list all snapshots

```
cloudctl volume snapshot list [flags]
```
...
```
  -h, --help               help for list
      --id string          snapshotid to filter [optional]
      --name string        name to filter
      --partition string   partition to filter [optional]
      --project string     project to filter
      --sort-by strings    sort by (comma separated) column(s), sort direction can be changed by appending :asc or :desc behind the column identifier. possible values: id|partition|tenant|volume_name
```
