## cloudctl s3 client-config

returns fitting configuration of an s3 user for given client

```
cloudctl s3 client-config <id> [flags]
```

### Options

```
      --for-client string   output suitable client configuration for either minio|s3cmd (default "minio")
