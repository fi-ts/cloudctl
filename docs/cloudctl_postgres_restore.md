## cloudctl postgres restore

restore postgres from existing one

```
cloudctl postgres restore [flags]
```
...
```
  ...
  -h, --help                        help for restore
      ...
      --partition string            partition where the database should be created. Changing the partition compared to the source database requires administrative privileges
      --source-postgres-id string   if of the primary database
      --timestamp string            point-in-time to restore to, defaults to current system time
      --version string              postgres version of the database
```
